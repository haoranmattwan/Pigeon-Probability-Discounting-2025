...
raw_data_exp1 <- read_csv("PigeonPD_Exp1.csv")
raw_data_exp2 <- read_csv("PigeonPD_Exp2.csv")
...
model_exp1_16p <- brm(
  ...
  file = "rds/bf_mod1.rds" # Cache model fit
)
...
model_exp1_32p <- brm(
  ...
  backend = "cmdstanr", 
  control = list(adapt_delta = 0.95, max_treedepth = 10),
  file = "rds/bf_mod2.rds" # Cache model fit
)
...
# 16-Pellet Models
model_exp2_16p_m100 <- fit_brm_model_exp2(filter(filtered_data_exp2, Amount == 16, Multiplier == 1.00), "rds/bf_mod3a.rds")
model_exp2_16p_m75  <- fit_brm_model_exp2(filter(filtered_data_exp2, Amount == 16, Multiplier == 0.75), "rds/bf_mod3b.rds")
model_exp2_16p_m25  <- fit_brm_model_exp2(filter(filtered_data_exp2, Amount == 16, Multiplier == 0.25), "rds/bf_mod3c.rds")

# 32-Pellet Models
model_exp2_32p_m100 <- fit_brm_model_exp2(filter(filtered_data_exp2, Amount == 32, Multiplier == 1.00), "rds/bf_mod4a.rds")
model_exp2_32p_m75  <- fit_brm_model_exp2(filter(filtered_data_exp2, Amount == 32, Multiplier == 0.75), "rds/bf_mod4b.rds")
model_exp2_32p_m25  <- fit_brm_model_exp2(filter(filtered_data_exp2, Amount == 32, Multiplier == 0.25), "rds/bf_mod4c.rds")
```
